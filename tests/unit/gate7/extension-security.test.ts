...
import { ExtensionSecurityValidator } from "../../../src/gates/gate7-extensions/validators/extension-security.js";
...
describe("ExtensionSecurityValidator", () => {
  const validator = new ExtensionSecurityValidator();

  it("passes for clean extension", async () => {
    const result = await validator.validate(makeCtx(join(FIXTURES, "valid-extension")));
    expect(result.severity).toBe(Severity.PASS);
  });

  it("detects hardcoded AWS key in extension source", async () => {
